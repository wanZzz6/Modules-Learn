...
​	如果是下载的历史版本，后缀为 `.zip`，在"访达"页面解压完后双击运行，如果是进入了软件主界面而不是提示拖动到 Applications 中，你需要关闭软件，手动将安装包拖动到访达左侧的 Applications(应用程序)中
...
3. 安装完成后，在启动台（开始菜单）中找到Gitkraken图标，并点击运行。
...
>你可能遇到软件**已损坏**的问题，请参考：[解决方法](https://zhuanlan.zhihu.com/p/114919138)
...
可能是在安装Gitkraken时，你有正在使用的翻墙代理软件，软件自动将其设置成了 git 代理，可通过以下命令查看
